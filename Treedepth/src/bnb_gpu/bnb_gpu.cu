...
__global__ void GenerateKernel(int* const buf,
                               int* stack_head,
                               int* pending_reads,
                               int const n,
                               int const* const offsets,
                               int const* const out_edges,
                               int* best_td) {
  // wez ze stosu perermutacje
  auto my_perm_index = atomicAdd(stack_head, -1) - 1;
  extern __shared__ int8_t buf_shared[];
  int8_t* my_perm = &buf_shared[threadIdx.x * n];
  for (int i = 0; i < n; ++i)
    my_perm[i] = -1;
  int perm_len = ReadPermutation(buf, n, my_perm_index, my_perm);
  // sprawdz co o niej wiesz
  // drzewo eliminacji w tablicy
  int8_t* component_depth_info =
...
__global__ void BruteForceKernel(int* const buf,
  ...
  // for (int i = 0; i < blockDim.x; ++i) {
  //   if (threadIdx.x == i && cur_perm_len < 4) {
  //     for (int j = 0; j < n; ++j)
  //       printf("%d ", my_perm[j]);
  //     printf("\n");
  //   }
  ...
  // }
  for (int i = 0; i < blockDim.x; ++i) {
    if (threadIdx.x == i && cur_perm_len < 2) {
      for (int j = 0; j < n; ++j)
        printf("%d ", my_perm[j]);
      printf("\n");
    }
...
void BnBGPU::Run(BoostGraph const& g, std::size_t heur_td) {
  ...
  thrust::device_vector<int> pending_reads(1, 0);
    ...
    while (stack_head[0] > 0 &&
      ...
      if (sh < kThreads) {
        GenerateKernel<<<1, sh, 3 * sh * n * sizeof(int8_t)>>>(
            ...
            thrust::raw_pointer_cast(stack_head.data()),
            thrust::raw_pointer_cast(pending_reads.data()), n,
