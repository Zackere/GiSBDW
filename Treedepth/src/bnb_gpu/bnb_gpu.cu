...
__global__ void BruteForceKernel(int* const buf,
  ...
  while (cur_perm_len > perm_len) {
    if (cur_perm_len == n) {
      int max_td = 0;
      for (int i = 0; i < n; ++i)
        max_td = max(max_td, component_depth_info[i]);
      atomicMin(best_td, max_td);
      --cur_perm_len;
      continue;
    }
    // if (LowerBound(component_belong_info, component_depth_info, ncomponent,
    // n,
    //                offsets, out_edges) >= *best_td) {
    //   continue;
    // }
...
void BnBGPU::Run(BoostGraph const& g, std::size_t heur_td) {
  ...
  while (stack_head[0] > 0) {
    while (kThreads * kBlocks * n < buf.size() / n - stack_head[0] &&
           stack_head[0] > 0) {
      int sh = stack_head[0];
