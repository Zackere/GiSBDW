...
namespace td
{
	...
	class QuasiSetArray :
		public QuasiSetBase<UnsignedIntegral>
	{
	public:
		...
		{
#ifdef TD_QUASI_SET_CEHCK_OBJECT_STATE
			if (isElementExcluded_) throw std::logic_error("Only one element can be excluded at the same time");
			if (index > this->numberOfElements_) throw std::out_of_range("Index was bigger than number of elements in container");
		...
		{
#ifdef TD_QUASI_SET_CEHCK_OBJECT_STATE
			if (!isElementExcluded_) throw std::logic_error("Cannot recover excluded element, because no element was excluded");
			isElementExcluded_ = false;
#endif
		...
		{
#ifdef TD_QUASI_SET_CEHCK_OBJECT_STATE
			if (!isElementExcluded_) throw std::logic_error("Call to EncodeExcluded when no element was excluded");
#endif
			size_t code = 0;
			Element k = 1;
