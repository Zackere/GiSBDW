...
\begin{itemize}
	...
	\item $T_\varepsilon$ is represented by $G$ alone beacuse that is how the graph looks when no vertices were eliminated
	\item $T_w$ is a tree with leaves being induced subgraphs of $G$. Internal nodes are single vertices from $G$ which are also incuded subgraphs of $G$
	...
	\item Path from  root to leaf $H$ shows which vertices had to be removed in order to acquire $H$ as a connected conponent
