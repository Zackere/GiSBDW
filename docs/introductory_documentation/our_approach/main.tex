...
There are some observations that can be made to familliarize oneself with incomplete elimiation $w$ and $T_w$:
\begin{itemize}
	...
	\item $T_w$ is a tree with vertices being induced subgraphs of $G$
	\item $T_\varepsilon$ is represented by $G$ alone beacuse that is how the graph looks when no vertices were removed
	\item $T_w$ root is $G$ when $w=\varepsilon$ otherwise it's first letter of $w$
	\item Construction of $T_{wv}$ breaks down one of the leaves into connected components of a graph. Those new leaves are one step lower than its ancestor, to show how lower bound of their treedepth has changed upon elimination of $v$
	\item Path from leaf $H$ to root shows which vertices had to be removed in order to acquire $H$ as a connected conponent
...
Note: Upon elemination, one can find leaf $H$ in $T_w$ which maximizes $\left|H\right| + depth_{T_w}(H)$. Then $\left|H\right| + depth_{T_w}(H)$ is an upper-bound of $td(G)$, possibly improving upon it.
\subsubsection{Remarks}
